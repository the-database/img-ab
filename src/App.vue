<script setup>
  import { ref, computed, watch, onMounted, defineExpose } from 'vue';
  import InertiaDrag from 'inertia-drag';
  
  const allImages = ref([
    // 'https://external-preview.redd.it/5gMPrBEm3ienyaYZR9ZEer76aaPtJ-lHo7hKHILg0Zo.png?auto=webp&s=1d0dd98e594f4996b20da3b2a278b8519ad3e69a',
    'https://i.slow.pics/bmnXcYVG.png',
    'https://i.slow.pics/hcGpSScN.png',
    'https://i.slow.pics/ZeuCLk82.png',
// 'data:image/png;base64,'
    // 'https://i.slow.pics/wsXgXlcT.png',
    // 'https://i.slow.pics/j6H9Q1Ls.png',
    // 'https://upload.wikimedia.org/wikipedia/commons/thumb/b/b6/Image_created_with_a_mobile_phone.png/1920px-Image_created_with_a_mobile_phone.png'
  ]);

  const allImageNames = computed(() => allImages.value.map(path => path.split('\\').pop().split('/').pop()));

  const image = ref(null);
  const imageContainer = ref(null);

  const selectedImageIndex = ref(0);

  const nearestNeighborSampling = ref(false);
  const imageRendering = computed(() => nearestNeighborSampling.value ? 'pixelated' : 'optimizeQuality');

  const showInfo = ref(false);
  const showHelp = ref(false);

  const scrollXBy = ref(0);
  const scrollYBy = ref(0);

  const innerHeight = ref(0);
  const innerWidth = ref(0);

  const modeFitToWidth = ref(false);
  const modeFitToHeight = ref(false); // revert

  const scalingFactor = ref(1.0);

  // const position = { x: 0, y: 0 };
  const positionX = ref(0);
  const positionY = ref(0);

  let elRect = null;
  let offsetLeft = 0;
  let offsetTop = 0;
  let containerWidth = 0;
  let containerHeight = 0;

  let minX = containerWidth - image.value?.offsetWidth;
  let minY = containerHeight - image.value?.offsetHeight;
  const maxX = 0;
  const maxY = 0;

  let inertiaDrag = null;

  const useScalingFactor = computed(() => modeFitToHeight.value === false && modeFitToWidth.value === false);

  const onMove = function ( event ) {
    const elRect = imageContainer.value?.getBoundingClientRect();
    const dx = event.deltaX + elRect.left - offsetLeft;
    const dy = event.deltaY + elRect.top - offsetTop;
    // position.x = clamp( position.x + dx, minX, maxX );
    // position.y = clamp( position.y + dy, minY, maxY );
    positionX.value = clamp(positionX.value + dx, minX, maxX);
    positionY.value = clamp(positionY.value + dy, minY, maxY);

    // image.value.style.transform = `translate(${position.x}px, ${position.y}px) scale(${scalingFactor.value})`;

  };

  function clamp(value, min, max) {
    return Math.max( min, Math.min(max, value));
  }

  function stopCSSTransition() {
    image.value.style.transform = window.getComputedStyle(image.value).transform;
    image.value.style.transition = 'none';
    const matrix = image.value.style.transform.match( /-?\d+\.?\d*/g );
    console.log('matrix', matrix, image.value.style.transform);
    positionX.value = + matrix[ 4 ];
    positionY.value = + matrix[ 5 ];

  }

  function moveTo(x, y, enableTransition) {
    const tx = clamp(containerWidth / 2 - x, minX, maxX);
    const ty = clamp(containerHeight / 2 - y, minY, maxY);

    console.log('txty', tx, ty)

    inertiaDrag.forceDragEnd();
    inertiaDrag.stop();
    if( enableTransition ) image.value.style.transition = 'transform 1s';
    image.value.style.transform = 'translate( ' + tx + 'px, ' + ty + 'px )';
    }

  watch(image, ( newValue, oldValue ) => {
    console.log('watch image?', image.value)
    if (image.value) {

      image.value.onload = () => {
        elRect = imageContainer.value.getBoundingClientRect();
        offsetLeft = elRect.left;
        offsetTop = elRect.top;
        containerWidth = elRect.width;
        containerHeight = elRect.height;

        console.log('cwh', containerWidth, containerHeight)

        inertiaDrag = new InertiaDrag(imageContainer.value);

        minX = containerWidth - image.value?.offsetWidth;
        minY = containerHeight - image.value?.offsetHeight;

        moveTo(image.value.naturalWidth, image.value.naturalHeight, false);
        console.log('moveto?', image.value.naturalWidth , image.value.naturalHeight)
        inertiaDrag.addEventListener( 'dragstart', stopCSSTransition );
        inertiaDrag.addEventListener( 'dragmove', onMove );
        inertiaDrag.addEventListener( 'inertiamove', onMove );
        image.value.onload = null;
      }

    }
  }, {
    immediate: true,
  });

  watch(() => image.value?.containerWidth, (newWidth, oldWidth) => {
    console.log("YES!!!", image.value?.width, image.value?.height)
  }, {
    immediate: true
  });

  onMounted(() => {

    window.addEventListener('keydown', (e) => {
      console.log("keydown", e.key);
      if (isFinite(e.key)) {
        const numberPressed = parseInt(e.key);
        if (numberPressed <= allImages.value.length && numberPressed > 0) {
          selectedImageIndex.value = numberPressed - 1;
        }
      }

      let transform;

      switch (e.key) {
        case "ArrowLeft":
            // Left pressed
            break;
        case "ArrowRight":
            // Right pressed
            break;
        case "ArrowUp":
            // Up pressed
            break;
        case "ArrowDown":
            // Down pressed
            break;
        case "i":
          showInfo.value = !showInfo.value;
          break;
        case "h":
          showHelp.value = !showHelp.value;
          break;
        case "q":
          modeFitToHeight.value = false;
          modeFitToWidth.value = false;
          scalingFactor.value = 1;
          break;
        case "w":
          modeFitToHeight.value = false;
          modeFitToWidth.value = false;
          scalingFactor.value *= 2/3;
          break;
        case "e":
          modeFitToHeight.value = false;
          modeFitToWidth.value = false;
          scalingFactor.value *= 1.5;
          break;
        case "r":
          
          modeFitToWidth.value = true;
          modeFitToHeight.value = false;
          break;
        case "t":
          
          modeFitToHeight.value = true;
          modeFitToWidth.value = false;
          break;
        case "y":
          nearestNeighborSampling.value = !nearestNeighborSampling.value;
          break;
      }
    });
  });

  window.electronAPI?.handleArgs((event, value) => {
    allImages.value = value; // TODO revert
  })

</script>

<template>
  <main>
    <div ref="imageContainer" class="image-container">
      <span class="info" v-show="showInfo">
        {{ selectedImageIndex+1 }}/{{ allImages.length }}: {{  allImageNames[selectedImageIndex] }}
      </span>
      <table class="help" v-show="showHelp">
        <tbody>
          <tr>
            <td>h</td>
            <td>Show/Hide Help</td>
          </tr>
          <tr>
            <td>1-{{ allImages.length }}</td>
            <td>Select Image</td>
          </tr>
          <tr>
            <td>i</td>
            <td>Show/Hide Image Info</td>
          </tr>
          <tr>
            <td>q</td>
            <td>Image Zoom: 100% Image Size</td>
          </tr>
          <tr>
            <td>w</td>
            <td>Image Zoom: Zoom Out</td>
          </tr>
          <tr>
            <td>e</td>
            <td>Image Zoom: Zoom In</td>
          </tr>
          <tr>
            <td>r</td>
            <td>Image Zoom: Fit to Width</td>
          </tr>
          <tr>
            <td>t</td>
            <td>Image Zoom: Fit to Height</td>
          </tr>
          <tr>
            <td>y</td>
            <td>Image Sampling: Toggle smooth or nearest neighbor sampling</td>
          </tr>
        </tbody>
      </table>
      
      <img v-if="allImages.length > 0" id="image" ref="image" 
      :src="allImages[selectedImageIndex]" :class="{ 'fit-to-height': modeFitToHeight, 'fit-to-width': modeFitToWidth }" />
      <p class="none-message" v-else>No images loaded.</p>
    
    </div>
  </main>
</template>


<style scoped>
  .image-container .info {
    position: fixed;
    top: 1rem;
    left: 1rem;
    -webkit-text-stroke: 1px #333;
    z-index: 99;
    font-weight: bold;
    font-size:1.5rem;

    white-space: nowrap;
    overflow: hidden;
    text-overflow: ellipsis;
  }

  .image-container table.help {
    position: fixed;
    top: 4rem;
    left: 1rem;
    -webkit-text-stroke: 1px #333;
    z-index: 99;
    font-weight: bold;

    white-space: nowrap;
    overflow: hidden;
    text-overflow: ellipsis;
    width: 50%;
    font-family: inherit;
    font-size:1.5rem;
  }

  .image-container .debug {
    position: fixed;
    top: 4rem;
    left: 1rem;
    -webkit-text-stroke: 1px #333;
    z-index: 99;
    font-weight: bold;

    white-space: nowrap;
    overflow: hidden;
    text-overflow: ellipsis;
    width: 50%;
    font-family: inherit;
    font-size:1.5rem;
  }

  img.fit-to-height {
    height: 100vh !important;
  }

  img.fit-to-width {
    width: 100vw !important;
  }

  table, tbody, tr, td {
    font-size: inherit;
    font-family: inherit;
  }

  .image-container img {
    image-rendering: v-bind(imageRendering);
    display: block;
    transform: translate(v-bind(positionX)px, v-bind(positionY)px) scale(v-bind(scalingFactor));
    transform-origin: center;
  }

  .none-message {
    padding: 1rem;
  }

  .image-container {
    width: 100vw;
    height: 100vh;
    /* width: 800px; */
    /* height: 800px; */
    overflow:hidden;
    margin:0 auto;
    outline: 1px solid red;
  }
</style>
